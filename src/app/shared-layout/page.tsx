      ...
      {activeGame ? (
        <>
          ...
          <div className="active-game">
            <motion.div layout layoutId={`game-${activeGame.title}`} className="inner" ref={ref} style={{ borderRadius: 12 }}>
              <div className="header">
                ...
                <div className="header-inner">
                  <div className="content-wrapper">
                    <motion.h2 layoutId={`title-${activeGame.title}`} className="game-title">{activeGame.title}</motion.h2>
                    <motion.p layoutId={`description-${activeGame.title}`} className="game-description">{activeGame.description}</motion.p>
                  </div>
                  <motion.button layoutId={`button-${activeGame.title}`} className="button">Get</motion.button>
                </div>
              </div>
              <motion.p 
                layoutId={`title-${activeGame.longDescription}`}
                transition={{ duration: 0.2 }}
                className="long-description">{activeGame.longDescription}
              </motion.p>
            </motion.div>
          </div>
        </>
          ...
          {GAMES.map((game) => (
            ...
            >
              ...
              <div className="game-wrapper">
                <div className="content-wrapper">
                  <motion.h2 layoutId={`title-${game.title}`} className="game-title">{game.title}</motion.h2>
                  <motion.p layoutId={`description-${game.title}`} className="game-description">{game.description}</motion.p>
                </div>
